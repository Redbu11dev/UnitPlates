...
UPLibAuraDurationsByRank = {
  ...
  ['Chill']={[0]=30.0,},
  ['Chill Nova']={[0]=10.0,},
  ['Chilled']={[0]=1.5,},--there are 2 spells named "chilled?" --if I receive no rank in combat log, then it's probably not cast by player?
  ...
  --['Rend']={[1]=1.0,[2]=12.0,[3]=15.0,[4]=18.0,[5]=21.0,[6]=21.0,[7]=21.0,[0]=21.0,},
  ['Rend Flesh']={[0]=12.0,},
  ['Repentance']={[0]=6.0,},
...
function UPLibAuraDurationsGetAuraDuration(name, rankNumber)
	...
	if not UPLibAuraDurationsByRank[name] then
		--name not found
	...
	if not duration then
		--try rank 0, as many auras share common duration and it seems to be meant to use rank 0 if so
		--but should probably still soft notify that the rank is not handled
		duration = UPLibAuraDurationsByRank[name][0]
		if duration then
			--print("------ UNITPLATES (soft notify): aura rank not found for name: "..name.."/rank: "..rankNumber.." - falling back to rank 0 duration")
			UPDebugStoreUPDebugMissingSpellsEntry(name, rankNumber)
		end
	end
	
	--update durations depending on YOUR combopoints and talents
	if duration then
		--TODO should probably compare guid with target, and only apply these corrections if it is your target?
		--But no idea what to do with AoE like "Demoralizing Shout"
		--ALSO it is like super wrong to use GetTalentInfo(3,7) because your class can be different, probably also should check if you are the correct class
		--Should not be using this until figured
		
		-- if name == UPLibAuraDurationsByRank["Rupture"] then --it can only be valid if you are applying it yourself
		  -- -- Rupture: +2 sec per combo point
		  -- duration = duration + GetComboPoints()*2
		-- elseif name == UPLibAuraDurationsByRank["Kidney Shot"] then --it can only be valid if you are applying it yourself
		  -- -- Kidney Shot: +1 sec per combo point
		  -- duration = duration + GetComboPoints()*1
		-- elseif name == UPLibAuraDurationsByRank["Demoralizing Shout"] then --it can only be valid if you are applying it yourself
		...
		-- elseif name == UPLibAuraDurationsByRank["Shadow Word: Pain"] then --it can only be valid if you are applying it yourself
		...
		-- elseif name == UPLibAuraDurationsByRank["Frostbolt"] then --it can only be valid if you are applying it yourself
		  -- -- Permafrost: +1s per talent
		  -- local _,_,_,_,count = GetTalentInfo(3,7)
		  -- if count and count > 0 then duration = duration + count end
		-- elseif name == UPLibAuraDurationsByRank["Gouge"] then --it can only be valid if you are applying it yourself
		  -- -- Improved Gouge: +.5s per talent
		  -- local _,_,_,_,count = GetTalentInfo(2,1)
		  -- if count and count > 0 then duration = duration + (count*.5) end
		-- end
	end
	--update durations depending on YOUR combopoints and talents end
	
	if duration == nil then
		--rank not found
		--report that rank for name is not in db
		--print("------ UNITPLATES: aura rank not found for name: "..name.."/rank: "..rankNumber)
		UPDebugStoreUPDebugMissingSpellsEntry(name, rankNumber)
	end
